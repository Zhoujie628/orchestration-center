...
class _NegotiationClient:
    def __init__(self, requested_fields):
        ...
        self._delegate = create_a2at_client()
...
def _proposal_request(task: TaskRequest, requested_field: str) -> NegotiationRequest:
    ...
    generated = create_a2at_server().generate_negotiation_propose_prompt_from_data(
        NegotiationProposeData(
            context,
            InformationProposeContent(
                [NegotiationItem(requested_field, "请补充该字段")],
                "AND",
            ),
        ),
        INFORMATION_NEGOTIATION_PROPOSE_URI,
    ...
    return NegotiationRequest(
        ...
        received=ReceivedMessage(task_metadata=generated.build_metadata_content()),
        previous_exchanges=(),
